...
module Sigstore
  module Verification
    ...
    class PolicySet < Literal::Struct
      ...
      def self._Lazy(type) # rubocop:disable Naming/MethodName
        ->(o) { !o.nil? && Verification.const_get(type) === o }
      end
      ...
      def accept_child(child_tag, child_id, child_pos)
        if child_tag == "def"
          @defs[child_id] = Macro.new(start: child_pos)
        elsif !child_tag.nil?
          raise "unknown tag #{child_tag} #{child_id} #{child_pos}"
        else
          case child_id
          when "description"
            DescriptionProxy.new(self)
          else
            policy = Policy.new(start: child_pos, type: child_id, name: "UGH")
            @policies << policy
            policy
          end
        end
    ...
    class Context < Literal::Struct
      ...
      def format_failures
        ...
        failures.map do |(policy_start, policy_end), failing_rules_by_location|
          ...
          context = policy_set.source.byteslice(policy_start, policy_end - policy_start) << "\n" <<
          ...
          "#{policy_set.filename || "<unknown>"}:#{policy_start}:#{policy_end}:error: " \
            "policy failed to match\n#{context}"
        end.join("\n")
      end
    end
    ...
    module Rule
      ...
      class OneOf < Literal::Struct
        include Rule

        def evaluate(ctx)
          ctx.add_failure(self) if rules.empty?
        ...
        def deconstruct_in_ctx(ctx)
          rules.to_h { [_1, _1.deconstruct_in_ctx(ctx)] }
        end
      end
    end
    ...
    class Policy < Literal::Struct
      ...
      def accept_argument(value)
        raise "#{self.class.name} only accepts a single argument" unless name == "UGH"
    ...
    class Macro < Literal::Struct
      ...
      def evaluate(ctx, props)
        ...
        props.each do |name, val|
          ...
          when Type::BOOLEAN
            raise TypeError, "expected boolean for #{name}, got #{value.inspect}" unless BooleanType === value
          when Type::INTEGER_P
